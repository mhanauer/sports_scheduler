...
from holidays import UnitedStates as holidays

def generate_dates(start_date, end_date, day_of_week):
    us_holidays = holidays(years=[start_date.year, end_date.year])
...
for league, matches_in_league in matches.items():
    ...
    for match in matches_in_league:
        print(f"{match['team1']} vs {match['team2']} at {match['court_location']} Courts {', '.join(str(x) for x in match['court_numbers'])} on {match['date_time'].strftime('%Y-%m-%d %I:%M %p')}")
